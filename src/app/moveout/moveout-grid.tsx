...
import type { Apartment, MoveoutItemWithSale } from "@/lib/types";
...
interface MoveoutGridProps {
  items: MoveoutItemWithSale[];
  apartments: Apartment[];
}

type SortKey = "newest" | "price_asc" | "price_desc";

export function MoveoutGrid({ items, apartments }: MoveoutGridProps) {
  const [categoryFilter, setCategoryFilter] = React.useState("all");
  const [conditionFilter, setConditionFilter] = React.useState("all");
  const [apartmentFilter, setApartmentFilter] = React.useState("all");
  ...
  const filtered = React.useMemo(() => {
    ...
    if (freeOnly)
      result = result.filter((i) => i.price === 0);
    else
      result = result.filter((i) => i.price <= maxPrice);
    if (apartmentFilter !== "all") {
      result = result.filter(
        (i) =>
          i.moveout_sales?.apartments?.id === apartmentFilter ||
          (apartmentFilter === "__custom" && !i.moveout_sales?.apartments),
      );
    }

    if (sort === "price_asc") result.sort((a, b) => a.price - b.price);
    else if (sort === "price_desc") result.sort((a, b) => b.price - a.price);
    // newest = default insertion order from server (already sorted by created_at desc)

    return result;
  }, [items, categoryFilter, conditionFilter, freeOnly, maxPrice, apartmentFilter, sort]);

  // Apartments present in these items
  const presentApartments = React.useMemo(() => {
    const ids = new Set(items.map((i) => i.moveout_sales?.apartments?.id).filter(Boolean));
    return apartments.filter((a) => ids.has(a.id));
  }, [items, apartments]);

  const hasCustom = items.some((i) => !i.moveout_sales?.apartments);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border bg-card p-4 shadow-sm">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-1.5">
            <Label className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Complex</Label>
            <Select value={apartmentFilter} onValueChange={setApartmentFilter}>
              ...
              <SelectContent>
                ...
                {presentApartments.map((a) => (
                  <SelectItem key={a.id} value={a.id}>{a.name}</SelectItem>
                ))}
                {hasCustom && <SelectItem value="__custom">Other location</SelectItem>}
              </SelectContent>
            </Select>
          </div>
        ...
        <div className="mt-4 flex flex-wrap items-end gap-6">
          ...
          {!freeOnly && (
            <div className="flex min-w-52 flex-1 items-end gap-3">
              ...
              <div className="w-20">
                <Input
                  ...
                  max={9999}
                  value={maxPrice}
                  onChange={(e) => setMaxPrice(Math.min(9999, Number(e.target.value)))}
                  className="h-9"
                />
              </div>
            </div>
      ...
      {filtered.length === 0 ? (
        ...
        <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filtered.map((item) => {
            ...
            const location =
              sale?.apartments?.name ?? sale?.custom_location ?? "Unknown location";
            const saleHref = `/moveout/${sale?.id ?? ""}`;
                ...
                >
                  ...
                  <div className="relative aspect-square w-full overflow-hidden bg-muted">
                    ...
                    <div className="absolute bottom-2 left-2">
                      <span className={`rounded-full px-2.5 py-1 text-sm font-bold shadow ${item.price === 0 ? "bg-green-500 text-white" : "bg-background/90 text-foreground backdrop-blur-sm"}`}>
                  ...
                  <div className="flex flex-1 flex-col gap-2 p-3">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${CONDITION_COLORS[item.condition]}`}>
